...
public class SessionCheckInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
		...
		try {
			...
			if (loginInfo != null && session != null) {
				if(userInfo != null) userInfo.setUserInfo(session);
			} else {
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
		// "/", "/I18N", "/main/login", "/favicon.ico" URL 요청에 대해서만 "X-Frame-Options=DENY" 옵션 적용
		// if (isSkipURI(request)) {
		//	   response.addHeader("X-Frame-Options", "DENY");
		// }
		
		// 웹스퀘어 화면 요청에 대해서는 모두 "X-Frame-Options=DENY" 옵션 적용
		// 만일 웹스퀘어 화면을 iframe으로 로딩해야하는 경우가 필요할 경우 예외 처리해야 함
		response.addHeader("X-Frame-Options", "DENY");
	}
	...
	private boolean isSkipURI(HttpServletRequest request) {
		String[] skipUrl = { "/", "/I18N", "/main/login", "/static/favicon.ico" };
