...
gcm._setRequestHeaderFunction = function(requestObj) {
	  // 아래의 JWT Token은 예시 데이터로 서버에서 생성해서 전달한 JWT Token을 보내야 합니다.
	  //var jwtToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ0b3B0YWwuY29tIiwiZXhwIjoxNDI2NDIwODAwLCJodHRwOi8vdG9wdGFsLmNvbS9qd3RfY2xhaW1zL2lzX2FkbWluIjp0cnVlLCJjb21wYW55IjoiVG9wdGFsIiwiYXdlc29tZSI6dHJ1ZX0.yRQYnWzskCZUxPwaQupWkiUzKELZ49eM7oWxAQK_ZXw";
	  // Request Header에 jwtToken 데이터가 추가되dj 전송됩니다.
	  //requestObj.setRequestHeader("jwtToken", jwtToken);
};
...
gcm.win.onpageload = function(obj){
	
	var paramData = obj.getParameter("paramData");
	
	console.log("페이지 최초 로딩시:History 에 저장된:검색Dataset(검색조건):" + JSON.stringify(paramData));
	
	var layout = $p.top().scwin.getLayoutId();
	
	if (layout == "T") {
		
		var menuCode = paramData.menuInfo["menuCode"];
		var tabId = $p.top().tac_layout.getSelectedTabID();
		...
		if ($p.top().tac_layout.getSelectedTabID() !== menuCode) {
			...
			tabId = $p.top().tac_layout.getTabID(tabIndex);
		...
		if(paramData.param){
			$p.parent().tac_layout.getWindow(tabId).dma_search.setJSON(paramData.param);
		}
		
	}else if (layout == "M") {
		
		var winFrame = $p.top().wdc_main.getWindow(paramData.menuInfo.menuCode);
		
		if(paramData && paramData.param){
			winFrame.dma_search.setJSON(paramData.param);
		}
		
	}else if (layout == "S") {
		
		if(paramData && paramData.param){
			$p.top().wfm_layout.getObj("dma_search").setJSON(paramData.param);
		}
	}
}

gcm.win.historySave = function(obj){
	var paramData = obj.getParameter("paramData");
	...
	if (layout == "T") {
		...
		paramData.param = $p.parent().tac_layout.getWindow(tabId).dma_search.getJSON();
		console.log("검색 버튼 클릭:검색Dataset(검색조건):" + JSON.stringify(paramData.param));
		
	}else if (layout == "M") {
		
		var winFrame = $p.top().wdc_main.getWindow(paramData.menuInfo.menuCode);
		
		paramData.param = winFrame.dma_search.getJSON();
		console.log("검색 버튼 클릭:검색Dataset(검색조건):" + JSON.stringify(paramData.param));
		
	}else if (layout == "S") {
		paramData.param = $p.top().wfm_layout.getObj("dma_search").getJSON();
		console.log("검색 버튼 클릭:검색Dataset(검색조건):" + JSON.stringify(paramData.param));
	}
	
	gcm.win.pushState(paramData);
	
	return paramData.param;
}


//gcm.win.sbm_searchMessage_submitdone = function(functionObj){
//	var options = functionObj.call();
//	
//	var paramData = options.parameter.getParameter("paramData");
//	var data = JSON.stringify(paramData);
//	
//	gcm.win.pushState(paramData);
//	
//	console.log("현재Page의 검색조건 저장 : pushState History:" + JSON.stringify(data));
//}
...
gcm.win.openMenu = function($p, menuNm, url, menuCode, paramObj, option) { 
	...
	if (com.util.isEmpty(url)) {
		...
		menuNm = menuNm.substr(0,10) + "...";

		if (layout == "T") {
			
			var tabObj = { 
				closable : closable, //탭 닫기 기능 제공
				openAction : "exist", // exist 는 기존 탭을 갱신, new 는 항상 새로, select는 동일 id 가 존재하면 선택, last: 기존 tab을 마지막 tab으로 이동후 선택
				//openAction : "new",
				label : menuNm
			};
...
gcm.win.openPopup = function($p, url, opt, data) {
	...
	var options = {
		...
		scrollbars : true,
		windowDragMove: true,
		frameModal : (opt.frameModal) ? opt.frameModal : "top",
		/*
		frameModal : (opt.frameModal) ? opt.frameModal : "top",
		title : (opt.title == true) ? true : false, 
		menubar : (opt.menubar == true) ? true : false, 
		useMaximize: (opt.useMaximize == true) ? true : false,
		foldOnMinimized: (opt.foldOnMinimized == true) ? true : false,
		controls: (opt.controls) ? opt.controls: null,
		useControl:(opt.useControl == true) ? true : false,
		useModalStack: (opt.useModalStack == true) ? true : false,
		modal: (opt.modal == true) ? true : false,		
		*/
...
gcm.win.messagBox = function($p, messageType, messageStr, closeCallbackFncName, opts) {
	...
	com.win.openPopup("/static/cm/xml/messageBox.xml", options, data);
};
...
gcm.win.pushState = function(data) {
	...
	console.log("History 저장(history.pushState):" + JSON.stringify(data));
...
gcm.win.changePageState = function() {
	...
	if(!com.util.isEmpty(history.state) && !com.util.isEmpty(history.state.data) && !com.util.isEmpty(history.state.data.menuInfo)){
		...
		data.param.isFromHistory = true;
		
		console.log("뒤로가기 Event 발생(History) : 이동할 Page의 검색조건 : popstate History:" + JSON.stringify(data));
